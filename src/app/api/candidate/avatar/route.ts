...
// Candidate profile photo upload: stores the image and saves it on the account.
export async function POST(request: Request) {
  ...
  try {
    ...
    const url = `/api/media/${result.insertedId.toString()}`;
    await db.collection("candidates").updateOne({ _id: new ObjectId(session.id) }, { $set: { avatarUrl: url } });
    return NextResponse.json({ ok: true, url });
  } catch (err) {
    console.error("[candidate avatar]", err);
